...
def upload_to(instance, filename):
    ...
    return 'users/comments/%s%s' % (
        timezone_now().strftime("%Y/%m/%Y%m%d%H%M%S"),
        filename_ext.lower(),
    )


class PictureCommentManager(CommentManager):
    """
    Manager to optimize SQL queries for comments.
    """

    def get_query_set(self):
        return super(CommentManager, self).get_query_set().select_related('user').select_related('picture')
...
class PictureComment(Comment):
    ...
    objects = PictureCommentManager()

    class Meta:
        proxy = False
        ordering = ['-submit_date']

    picture = ForeignKey(UploadedFile, blank=True, null=True, on_delete=models.CASCADE)

    def save(self, *args, **kwargs):
        super(PictureComment, self).save(*args, **kwargs)


enable_voting_on(PictureComment)
...
def on_comment_posted(sender, comment, request, **kwargs):
    ...
    if not appsettings.PICTURE_COMMENTS_USE_EMAIL_NOTIFICATION:
        return
...
@receiver(pre_delete, sender=PictureComment, dispatch_uid='post_delete_comment')
def pre_delete_comment(sender, instance, using, **kwargs):
    Notification.objects.filter(action_object_object_id=instance.id).delete()
...
class CommentsRelation(GenericForeignKey):
    """
