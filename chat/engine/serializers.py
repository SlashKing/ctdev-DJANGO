...
class GroupChatUserSerializer(ChatUserSerializer):
    ...
    class Meta:
        model = GroupChatUser
        fields = ChatUserSerializer.Meta.fields + ('admin', 'activated')
...
class RoomSerializer(ModelSerializer):
    ...
    notifications_count = SerializerMethodField()
    ...
    class Meta:
        model = Room
        fields = ('id', 'messages', 'users', 'name', 'active', 'notifications_count')
...
#
   ...
   #     return UserSerializer(obj.requested, context={'request':self.context.get('request',None)}).data

    class Meta:
        model = JoinRequest
        fields = ('id', 'requester', 'requested', 'is_rejected', 'is_viewed', 'created', 'message')

class JoinRequestWithRoomSerializer(JoinRequestSerializer):
    room = SerializerMethodField()

    def get_room(self, obj):
        return GroupRoomSerializer(
            GroupRoom.through_objects.by_id(
                obj.content_object.id).prefetch().first(),
            context={'request':self.context.get('request',None)}
        ).data

    class Meta:
        model = JoinRequest
        fields = JoinRequestSerializer.Meta.fields + ('room',)
