...
class CicuUploaderInput(forms.ClearableFileInput):
    ...
    def __init__(self, attrs=None, options=None):
        ...
        self.options += (options.get('modalButtonLabel', ("<i class='glyphicon glyphicon-camera' style='font-size:45px;'></i>")),)
        ...
        self.options += (options.get('fileUploadLabel', ("<i class='glyphicon glyphicon-camera' style='font-size:45px;'></i>")),)

    def render(self, name, value, attrs=None):
        attrs = attrs or {}
        if value:
            filename = u'%s%s' % (settings.MEDIA_URL, UploadedFile.objects.get(pk=value))
        else:
            filename = ''
        attrs.update({
